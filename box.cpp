	...
	const int r)
{
	...
	const int x1 = std::min((x<=r)?0:x-r, img.width());
	const int y1 = std::min((y<=r)?0:y-r, img.height());
	const int x2 = std::min(x+r, img.width());
	const int y2 = std::min(y+r, img.height());
